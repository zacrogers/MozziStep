...
const char BTN_3 = 5;
const char BTN_4 = 4;
const char SEQ_TOG = 3;
const char OCT_TOG = 10;
const char OSC_SEL = 8;
const char MODE_SEL = 11;
const char ROOT_SEL = 12;
...
/* Set freq sequqnce based on current steps, getting current index sequence, then getting the freq from that index in current mode */
void set_mode(){
    current_mode ++;
    if(current_mode == 7){current_mode = 0;}
    for(int i = 0; i < 8; i++){ sequence[i] = modes[current_mode][sequence_steps[i]];}
}
...
void set_mux(int seq_point){
    PORTC &= MUX_MASK;
    if(seq_point == 0){PORTC &= MUX_MASK;
        PORTC |= B0001000;}       //s0=0, s1=1, s2=0
    else if(seq_point == 1){PORTC &= MUX_MASK;
        PORTC |= B00000100;} //s0=1, s1=0, s2=0
    else if(seq_point == 2){PORTC &= MUX_MASK;
        PORTC |= B00000000;} //s0=0, s1=0, s2=0
    else if(seq_point == 3){PORTC &= MUX_MASK;
        PORTC |= B00001100;} //s0=1, s1=1, s2=0
    else if(seq_point == 4){PORTC &= MUX_MASK;
        PORTC |= B00010100;} //s0=1, s1=0, s2=1
    else if(seq_point == 5){PORTC &= MUX_MASK;
        PORTC |= B00011100;} //s0=1, s1=1, s2=1
    else if(seq_point == 6){PORTC &= MUX_MASK;
        PORTC |= B00011000;} //s0=0, s1=1, s2=1
    else if(seq_point == 7){PORTC &= MUX_MASK;
        PORTC |= B00010000;} //s0=0, s1=0, s2=1
}
